const {
  LoginFlowTypes: { REGISTER, VALID_LOGIN },
} = require('../consts');
const {
  LOGIN_FLOW,
  ID_TOKEN,
...
const AccountStatuses = new Map()
  .set(REGISTER, 'NOT_FOUND')
  .set(VALID_LOGIN, 'CONFIRMED');

const AuthenticationStates = new Map()
  .set(REGISTER, 'UNREGISTERED')
  .set(VALID_LOGIN, 'VERIFIED');
...
module.exports = function (app) {
  ...
  app.post('/login/provisioning/:id/check-dob', (_, res) => {
    res.status(200);
    res.end();
  });

  app.post('/login/provisioning/:id/create', (_, res) => {
    res.status(200);
    res.end();
  });
};
